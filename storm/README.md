...
(EVG 3.0 Storm). Copied from Ranger `lib/evg` at commit recorded in
[`ORIGIN.sha`](ORIGIN.sha).

This directory is a Ranger 3.x package named `evg`. A Ranger application
imports it with `Import "pkg:evg/EVGElement.rgr"` after depending on this
repository:
...
or, next to a checkout, `"evg": { "path": "../evg/storm" }`.

The TypeScript / NPM face (`import { EVG } from "evg"`) lives in `src/`
one directory up and still speaks the Thunderstruck 2.x XML/PDF API.
...
EVG is the package `evg`. This directory **is** that package. Outside
this repository, `rgrc install` fetches it from Git:
...
Its own dependency is `image` (`../image`: the JPEG and PNG codecs behind
`EVGImageDecode`), which depends on `zip` (`../zip`: DEFLATE). Both are
sibling path dependencies, so a Git fetch of `subdir: "storm"` brings
them along at the same commit. Nothing under this directory imports
Ranger `gallery/`.

The window layer that paints through the gallery's software rasteriser
(`EVGWindow`, `EVGTextFit`, `EVGContextMeasurer`, `EVGRulerView`,
`EVGToolbarView`) stays in Ranger as `gallery/evg_window`.

Painters that still live in Ranger (PDF, PNG, HTML from
`gallery/pdf_writer`) consume the display list this engine produces.
The hosts in `gl/`, `html/`, `android/` and `apple/` moved here with the
engine.

## License

**MIT.** EVG moved from Ranger `gallery/evg` to `lib/evg` (and from AGPL
to MIT) in September 2026, then here to `terotests/evg` as 3.0 Storm.
The TypeScript Thunderstruck API in `src/` is MIT as well. See
[`LICENSE`](../LICENSE).
